...
from muzakbot.settings import ChatSettings, MuzakbotSettings, HandlerStrategy
from muzakbot.thebot import SongLinkHandler, FoundData
...
@pytest.mark.parametrize(
    ...
    [
        (HandlerStrategy.COMMAND, '/links https://open.spotify.com/test',
            FoundData('https://open.spotify.com/test', '/links https://open.spotify.com/test')),
        (HandlerStrategy.COMMAND, 'https://open.spotify.com/test', None),
        (HandlerStrategy.MENTION, {'text': 'https://open.spotify.com/test', 'parts': [factories.mention(123456)]},
            FoundData('https://open.spotify.com/test', 'https://open.spotify.com/test')),
        (HandlerStrategy.MENTION, {'text': 'https://open.spotify.com/test', 'parts': [factories.mention(123455)]}, None),
        (HandlerStrategy.MENTION, 'https://open.spotify.com/test', None),
        (HandlerStrategy.ALL, 'https://open.spotify.com/test',
            FoundData('https://open.spotify.com/test', 'https://open.spotify.com/test')),
...
@pytest.mark.parametrize(
    ...
    [
        (['spotify'], 'https://open.spotify.com/test',
            FoundData('https://open.spotify.com/test', 'https://open.spotify.com/test')),
        (['spotify', 'youtube'], 'Look at this cool video: youtube.com/video/dgfdfgfd \nI like it a lot',
            FoundData('youtube.com/video/dgfdfgfd', 'Look at this cool video: youtube.com/video/dgfdfgfd \nI like it a lot')),
...
def test_forwarded_message(handler):
    event = factories.event({'parts': [factories.part('forward', message={'text': 'https://open.spotify.com/test'})]})
    assert FoundData('https://open.spotify.com/test', 'https://open.spotify.com/test') == handler.check_for_data(event)


def test_replied_message(handler):
    event = factories.event({'parts': [factories.part('reply', message={'text': 'https://open.spotify.com/test'})]})
    assert FoundData('https://open.spotify.com/test', 'https://open.spotify.com/test') == handler.check_for_data(event)
...
def test_custom_chat_settings(handler):
    ...
    event = factories.event({'text': 'https://youtube.com/test', 'chat': {'chatId': 'custom'}})
...
def test_send_message(handler):
    ...
    handler.bot.assert_last_call_kwargs(
        chat_id='test', text='Kid Cudi - Kitchen', reply_msg_id=999,
        inline_keyboard_markup=[[
            {"text": "Apple Music", "url": "https://music.apple.com/us/album/kitchen/1443108737?i=1443109064&uo=4&app=music&ls=1&at=1000lHKX"},
            {"text": "Spotify", "url": "https://open.spotify.com/track/0Jcij1eWd5bDMU5iPbxe2i"},
            {"text": "Youtube", "url": "https://www.youtube.com/watch?v=w3LJ2bDvDJs"},
        ]]
    )
