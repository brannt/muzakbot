...
from muzakbot.utils import chunks, uncamelize
...
class SongLinkHandler:
    ...
    def build_url_re(self, domains: list[str]) -> re.Pattern[str]:
    ...
    def get_chat_settings(self, event: Event) -> ChatSettings:
        return (
            self.settings.chats.get(event.from_chat)
            or self.settings.chats["default"]
        )

    def get_text(self, event: Event) -> Optional[str]:
        if "text" in event.data:
            return event.data["text"]  # type: ignore
        if "parts" in event.data:
            return next(
                (
                    p["payload"]["message"].get("text")
                    for p in event.data["parts"]
                    if "message" in p["payload"]
                ),
                None,
            )
        return None
    ...
    def has_mention(self, event: Event) -> bool:
        return "parts" in event.data and any(
            p["type"] == "mention" and p["payload"]["userId"] == self.bot.uin
            for p in event.data["parts"]
        )
    ...
    def check_for_data(self, event: Event) -> Optional[FoundData]:
        ...
        if (
            settings.handler_strategy == HandlerStrategy.MENTION
            and not self.has_mention(event)
        ):
            return None
        if (
            settings.handler_strategy == HandlerStrategy.COMMAND
            and self.links_command not in text
        ):
            return None
    ...
    def get_keyboard(
        ...
        links: Collection[tuple[str, dict[str, Any]]],
...
def start_bot(settings: MuzakbotSettings) -> None:
    ...
    bot.dispatcher.add_handler(
        MessageHandler(callback=SongLinkHandler(bot, settings))
    )
    bot.start_polling()
    bot.idle()
